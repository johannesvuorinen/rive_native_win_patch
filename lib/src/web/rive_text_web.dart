...
class TextShapeResultWasm extends TextShapeResult {
  ...
  BreakLinesResult breakLines(
    ...
    const lineSize = 32;
    var paragraphsList = RiveWasm.heapDataView(rawResult).readDynamicArray(0);
...
extension ByteDataWasm on ByteData {
  WasmDynamicArray readDynamicArray(int offset) {
    final ptr = getUint32(offset, Endian.little);
    final size = getUint32(offset + 4, Endian.little);
  ...
  Uint16List readUint16List(int offset, {bool clone = true}) {
    final array = readDynamicArray(offset);
    final list = RiveWasm.heapViewU16(array.ptr, array.size);
    if (clone) {
  ...
  Uint32List readUint32List(int offset, {bool clone = true}) {
    final array = readDynamicArray(offset);
    final list = RiveWasm.heapViewU32(array.ptr, array.size);
    if (clone) {
  ...
  Float32List readFloat32List(int offset, {bool clone = true}) {
    final array = readDynamicArray(offset);
    final list = RiveWasm.heapViewF32(array.ptr, array.size);
  ...
  Float32List readVec2DList(int offset, {bool clone = true}) {
    ...
    var list = RiveWasm.heapViewF32(array.ptr, array.size * 2);
...
class WasmDynamicArray {
  ...
  ByteData get data => RiveWasm.heapDataView(
      ptr); // Don't pass size here as that's the size of the dynamic object.
}
...
class LinesWasm extends ListBase<GlyphLineWasm> {
  ...
  GlyphLineWasm operator [](int index) {
    ...
    return GlyphLineWasm(
      RiveWasm.heapDataView(
        wasmDynamicArray.ptr + index * lineSize,
      ),
    );
  }
...
class FontWasm extends Font {
  ...
  TextShapeResult computeShape(
    ...
    var reader = BinaryReader(results);
  ...
  Iterable<FontTag> get features {
    ...
    return features
        .map((value) => FontTagWasm((value as js.JSNumber).toDartInt));
  }
...
class TextHelperWeb extends TextHelper {
  @override
  double convertStringToNumber(String value) => RiveWasm.toNativeString(
        value,
        (valuePointer) {
          return (_convertStringToNumber.callAsFunction(null, valuePointer)
                  as js.JSNumber)
              .toDartDouble;
        },
      );
